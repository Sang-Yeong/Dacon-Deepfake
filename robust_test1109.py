...
python train.py --kernel-type SBS_5fold_b5_35ep --k-fold 5 --data-folder sampled_face/ --enet-type tf_efficientnet_b5_ns --n-epochs 35 --batch-size 32
python robust_test1109.py --kernel-type SBS_5fold_b5_35ep --k-fold 5 --data-folder sampled_face/ --enet-type tf_efficientnet_b5_ns --n-epochs 35 --batch-size 32
...
def main():
    ...
    for attack_strength in attack_strengths:
        ...
        auc_max = run(0, df_train, meta_features, n_meta_features, transforms_train, transforms_val, target_idx)
        print(auc_max)
        auc_save.append(auc_max)
        with open(os.path.join("./noise_watch", f'log_{args.kernel_type}.txt'), 'a') as appender:
            appender.write('attack_strengths   :   ' + str(attack_strength) +'      auc   :   '+ str(auc_max) + '\n')
